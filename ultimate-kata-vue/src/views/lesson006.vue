  ...
  <div class="lesson">
    <div class="code-bg">
      <div class="prompt">
        ...
        <h2 class="white-txt">
          Check the <span class="string">'plants'</span> variable to see if it <span class="string">'is an Array'</span>
        </h2>
        <h2 class="white-txt">
          (Put in variable called <span class="string">'arrCheck'</span>)
        </h2>
      </div>
      ...
      <div class="attempt">
        <div class="greyed-out">
          ...
          <p><span class="func">let</span> <span class="vari">plants </span>= <span class="string">'Our sources are in place waiting for the signal...'</span>;</p>
          <p><span class="func">let</span> <span class="vari">arrCheck </span>= null;</p>
          <br />
          <p>console.<span class="method">log</span>(<span class="string"> 'plants is an array true or false?'</span> );</p>
        </div>
      ...
      <hr /><hr />
      <div class="greyed-out">
        <p>console.<span class="method">log</span>(<span class="string">'that's right... it is in fact: '</span> <span class="operator">+ </span><span class="vari">arrCheck</span> );</p>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      preLessonCode: `let plants = 'Our sources are in place waiting for the signal...';\nlet arrCheck = null;\nconsole.log('plants is an array true or false?' );\n// | |                   | |\n// v v your code is here v v\n\n`,
      lessonCode: `// type your code here //\n`,
      postLessonCode: "\n\n// ^ ^ your code is here ^ ^\n// | |                   | |\n\nconsole.log('that is right... it is in fact: ' + ( (arrCheck == false )||(arrCheck == true) ? arrCheck : (' oh wait... that seems odd')  ));\n",
  ...
  mounted() {
    ...
    localStorage.getItem('lesson6State') ? (this.aceEditor.session.setValue(localStorage.getItem('lesson6State'))) : this.aceEditor.session.setValue(this.lessonCode);
  ...
  methods: {
    blobber() {
      ...
      const tryIt = new Function(this.preLessonCode + lessonString + this.postLessonCode + 'return arrCheck' );
      let x = tryIt();
    ...
    saveLesson() {
      ...
      let lessonString = this.aceEditor.getValue();
      localStorage.setItem('lesson6State', lessonString)
    },
    submit() {
      ...
      const func = new Function(this.preLessonCode + lessonString + this.postLessonCode +'return arrCheck;' );
      let x = func();

      if ( x == false ) { // for some reason ( && typeof == Boolean ) doesn't return true >:-/
